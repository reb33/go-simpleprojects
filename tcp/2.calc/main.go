...
func calculate(input string) (float64, error) {
	...
	if len(parts) < 3 {
		return 0, fmt.Errorf("wrong formant, parts less then 3")
	}

	num1, err := strconv.Atoi(parts[0])
	if err != nil {
		return 0, fmt.Errorf("wrong formant, %s not number", parts[0])
	}

	num2, err := strconv.Atoi(parts[2])
	if err != nil {
		return 0, fmt.Errorf("wrong formant, %s not number", parts[2])
	}

	if len(parts[1]) != 1 || !strings.Contains("+-*/:", parts[1]) {
		return 0, fmt.Errorf("wrong formant, %s not in '+-*/:'", parts[1])
	}
	...
	default:
		return 0, fmt.Errorf("wrong formant")
	}
